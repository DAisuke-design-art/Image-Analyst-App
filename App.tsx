...
const App: React.FC = () => {
  ...
  const saveToNotion = async () => {
    ...
    try {
      // --- 修正ポイント3: 画像データのBase64ヘッダー削除 ---
      // data:image/png;base64,... の部分を削除して、純粋なBase64データのみにする
      ...
      // --- 修正ポイント2: カテゴリ・キーワードのマッピング ---
      const jpData = promptData.japanese;

      // Keywords用のリストを作成 (雰囲気、画風、感情などを結合)
      const keywordList = [
        jpData.VISUAL_STYLE.Vibe,
        jpData.VISUAL_STYLE.Artistic_Style,
        jpData.EMOTIONAL_PROFILE.Emotion,
        jpData.SCENE.Environment
      ].filter(Boolean); // 空の値を排除

      const payload = {
        // --- 修正ポイント1: 認証キーの追加 ---
        authKey: 'MySecretPass123',

        // GASが期待するキーへのマッピング
        subject: jpData.VISUAL_STYLE.Archetype || 'Image Analysis', // Notionのタイトル用
        category: jpData.VISUAL_STYLE.Archetype || 'Uncategorized', // NotionのCategory (Select)用
        keywords: keywordList.join(','), // NotionのKeywords (Multi-select)用
        details: jpData.fullPrompt, // Notionの本文用
